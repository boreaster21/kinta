...
<div class="attendance-detail stamp-correction-request-show">
    <h2 class="page-title">処理済み修正申請詳細</h2>
    ...
    <div class="approval-info">
        ...
        <div class="approval-info__item">
            ...
            <span class="approval-info__value">{{ \Carbon\Carbon::parse($request->approved_at)->format('Y年m月d日 H:i') }}</span>
        </div>
        @elseif($request->rejected_at)
         <div class="approval-info__item">
            <span class="approval-info__label">却下日時:</span>
            <span class="approval-info__value">{{ \Carbon\Carbon::parse($request->rejected_at)->format('Y年m月d日 H:i') }}</span>
        </div>
        @endif
         <div class="approval-info__item">
            <span class="approval-info__label">ステータス:</span>
            <span class="approval-info__value">
                @if($request->status === 'approved') <span style="color: green; font-weight:bold;">承認済み</span>
                @elseif($request->status === 'rejected') <span style="color: red; font-weight:bold;">却下</span>
                @endif
            </span>
        </div>
    </div>
    ...
    <div class="approval-actions">
        <a href="{{ route('stamp_correction_request.list', ['status' => 'processed']) }}" class="approval-button approval-button--back">一覧に戻る</a>
    </div>
</div>
